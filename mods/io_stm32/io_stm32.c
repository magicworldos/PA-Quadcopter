...
f32 est[6] = { 0, 0, 0, 0, 0, 0 };
f32 devi[6] = { 0, 0, 0, 0, 0, 0 };
...
void io_rc_data()
{
	...
	while (r)
	{
		...
		if (frame_parse_rc())
		{
			...
			{
				est[i] = controller_kalman_filter(est[i], ctl_est_devi, rc_data[i], ctl_measure_devi, &est_devi[i]);
			}
			...
			for (int i = 0; i < RCCH_COUNT; i++)
			{
				printf("%4d,%4d", rc_data[i], (u32)est[i]);
			}
			printf("\n");
		}
